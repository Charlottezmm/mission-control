...
const AGENT_ACTIVITY_POLL_INTERVAL_MS = 30000
...
export default function PixelOfficePage() {
  ...
  const supabaseTasksRef = useRef<Array<{ assignee?: string; status?: string; title?: string }>>([])
  ...
  useEffect(() => {
    ...
    const fetchAgents = async () => {
      try {
        const res = await fetch('https://hkarpznjtrhehauvcphf.supabase.co/rest/v1/tasks?select=assignee,status,title', {
          cache: 'no-store',
          headers: {
            apikey: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImhrYXJwem5qdHJoZWhhdXZjcGhmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI3MDAwNjQsImV4cCI6MjA4ODI3NjA2NH0.kXedOrtBkcXVc5s01MRA2sxdc1yDmFFi8TTskeqs0J0',
            Authorization: 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImhrYXJwem5qdHJoZWhhdXZjcGhmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI3MDAwNjQsImV4cCI6MjA4ODI3NjA2NH0.kXedOrtBkcXVc5s01MRA2sxdc1yDmFFi8TTskeqs0J0',
          },
        })
        if (!res.ok) throw new Error(`HTTP ${res.status}`)
        const tasks: Array<{ assignee?: string; status?: string; title?: string }> = await res.json()

        const AGENTS = [
          { name: 'Samantha', charIndex: 0, color: '#a855f7', emoji: '🦐' },
          { name: 'Beth', charIndex: 1, color: '#3b82f6', emoji: '🔧' },
          { name: 'Luna', charIndex: 2, color: '#ec4899', emoji: '✍️' },
          { name: 'Nova', charIndex: 3, color: '#f97316', emoji: '💡' },
          { name: 'Pixel', charIndex: 4, color: '#22c55e', emoji: '🎬' },
        ] as const

        const isWorking = (name: string) => tasks.some((task) => {
          const assignee = (task.assignee || '').trim().toLowerCase()
          const status = (task.status || '').trim().toLowerCase().replace(/_/g, '-')
          return assignee === name.toLowerCase() && status === 'in-progress'
        })

        const newAgents: AgentActivity[] = AGENTS.map((agent) => ({
          agentId: agent.name.toLowerCase(),
          name: agent.name,
          emoji: agent.emoji,
          state: isWorking(agent.name) ? 'working' : 'idle',
        ...
        supabaseTasksRef.current = tasks

        if (officeRef.current) {
          syncAgentsToOffice(newAgents, officeRef.current, agentIdMapRef.current, nextIdRef.current)
          // force fixed sprite index mapping char_0...char_4
          for (const agent of AGENTS) {
            const charId = agentIdMapRef.current.get(agent.name.toLowerCase())
            if (!charId) continue
            const ch = officeRef.current.characters.get(charId)
            if (!ch) continue
            ch.palette = agent.charIndex
            ch.hueShift = 0
          }
          cachedAgentIdMap = new Map(agentIdMapRef.current)
          cachedNextCharacterId = nextIdRef.current.current
              ...
              <>
                ...
                {hoveredInfo.isSubagent ? (
                  ...
                  <div className="space-y-1 text-[var(--text-muted)]">
                    {(() => {
                      const agentName = hoveredInfo.agent?.name ?? ''
                      const agentTasks = agents.find(a => a.name === agentName)
                      const state = agentTasks?.state ?? 'idle'
                      const activeTask = supabaseTasksRef.current.find((t) =>
                        (t.assignee || '').toLowerCase() === agentName.toLowerCase() &&
                        t.status === 'in-progress'
                      )
                      const allTasks = supabaseTasksRef.current.filter((t) =>
                        (t.assignee || '').toLowerCase() === agentName.toLowerCase()
                      )
                      return (
                        <>
                          <div className="flex justify-between gap-4">
                            <span>状态</span>
                            <span className={state === 'working' ? 'text-green-400' : 'text-yellow-400'}>
                              {state === 'working' ? '⚡ 工作中' : '😴 摸鱼中'}
                            </span>
                          </div>
                          <div className="flex justify-between gap-4">
                            <span>当前任务</span>
                            <span className="text-[var(--text)] text-right max-w-[140px] truncate">
                              {activeTask ? activeTask.title : '暂无进行中任务'}
                            </span>
                          </div>
                          <div className="flex justify-between gap-4">
                            <span>任务总数</span>
                            <span className="text-[var(--text)]">{allTasks.length} 个</span>
                          </div>
                          <div className="flex justify-between gap-4">
                            <span>待处理</span>
                            <span className="text-[var(--text)]">
                              {allTasks.filter((t: any) => t.status === 'backlog').length} 个
                            </span>
                          </div>
                        </>
                      )
